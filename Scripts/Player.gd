...
signal heath_updated(health)
signal killed()

export(PackedScene) var foot_step
export(PackedScene) var health_bar
...
func _set_health(value):
	...
	if health != prev_health :
		emit_signal("heath_updated", health)
		if health == 0 :
			kill()
			emit_signal("killed")
...
func attack():
	if Input.is_action_just_pressed("attack"):
		if is_on_floor():
			...
			$sword.play()
		if !is_on_floor():
			$attack/AnimationPlayer.play("attack1")
			$attack.visible = true
			$AnimatedSprite.visible = false
			$attack_Timer.start()
			$sword.play()
...
func _on_Hit_area_entered(area):
	if area.is_in_group("swor"):
		$health_bar.value -= 20
		ani_player.play("hit")
	if area.is_in_group("small_enemy"):
		$health_bar.value -= 10
		ani_player.play("hit")
	if area.is_in_group("body_dark"):
		$health_bar.value -= 10
		ani_player.play("hit")
	...
	if $health_bar.value <= 0:
		get_tree().change_scene("res://Objects/you_died.tscn")
	pass # Replace with function body.
